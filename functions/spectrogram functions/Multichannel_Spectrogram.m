...
for c = 1:numel(CN)
    for i = 1:size(SignalCell{1,c},2)
        %disp(['Processing channel: ', CN{c}, ', Sensor: ', num2str(i)]);
        %disp(['Input Signal size: ', num2str(size(SignalCell{1,c}(:,i)))]);
        [cell_MCS{1,c}(:,:,i),~,TimeGrids{1,c},FreqGrids{1,c}] = Spectrogram(SignalCell{1,c}(:,i),TimeCell{1,c},Channels.(CN{c}).sf,tw,to);
        %disp(['Output Spectrogram size: ', num2str(size(cell_MCS{1,c}(:,:,i)))]);
    end
end
...
for c = 1:numel(CN)
    if Channels.(CN{c}).sf ~= hsf
        ...
        for i = 1:size(cell_MCS{1,c},3)
            %disp(['Padding channel: ', CN{c}, ', Sensor: ', num2str(i)]);
            %disp(['Original size: ', num2str(size(cell_MCS{1,c}(:,:,i)))]);
            pad_MCS{1,c}(:,:,i) = matrix_padding(cell_MCS{1,c}(:,:,i),sz(1:2));
            %disp(['Padded size: ', num2str(size(pad_MCS{1,c}(:,:,i)))]);
        end
...
if numel(CN) == 1
    MCS = pad_MCS{1,1};  % If there is only one channel, directly assign it to MCS
else
    MCS = pad_MCS{1,1};  % Initialize MCS with the first channel
    for c = 2:numel(CN)
