...
for i = 1:Kfold
    for j = 1:size(Train.(FN{i}).data,1)
        disp(['Processing Train data: Fold ', num2str(i), ', Sample ', num2str(j)]);
        disp(['Input size: ', num2str(size(Train.(FN{i}).data(j,:)))]);
    ...
    for j = 1:size(Test.(FN{i}).data,1)
        disp(['Processing Test data: Fold ', num2str(i), ', Sample ', num2str(j)]);
        disp(['Input size: ', num2str(size(Test.(FN{i}).data(j,:)))]);
